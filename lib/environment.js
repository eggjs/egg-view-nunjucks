  ...
  [LOAD_FILTER]() {
    for (const unit of this.app.loader.getLoadUnits()) {
      ...
      for (const name of Object.keys(filters)) {
        this.addFilter(name, this[FILTER_WRAPPER](filters[name]), true);
      }
    }
  }

  [FILTER_WRAPPER](filterFunc) {
    return (...args) => {
      const callback = args.pop();
      // use Promise.resolve instead co, when support async/await
      co(filterFunc(...args)).then(
